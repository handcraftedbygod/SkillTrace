...
- **Supply-chain / dependency analysis.** `dependency_typosquat` now covers the first slice: install targets in `pip`/`npm`/`yarn` commands and bundled `requirements.txt`/`package.json` manifests get checked for name-similarity to a small, hand-curated list of popular packages (see [Explainability](#explainability)). Real-world attacks on npm/PyPI overwhelmingly happen via typosquatting, the dominant pattern this closes. Still open: a known-malicious package list (needs an external dataset that goes stale immediately — a real maintenance burden, deliberately not taken on yet), dependency confusion, and cross-checking what a skill actually pulls in against what it declares in a separate manifest.
