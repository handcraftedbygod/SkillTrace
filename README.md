...
| [`malicious/hidden-unicode-instruction`](examples/malicious/hidden-unicode-instruction) | "ASCII smuggling": a credential-exfil clause encoded in invisible Unicode tag characters (U+E0000 block), appended to an otherwise-ordinary `description` | Nothing to scan visually, the hidden clause renders as nothing in virtually any viewer — a description-only human review sees a clean skill | HIGH (score 14): `hidden_unicode_content` |
| [`malicious/hardcoded-secret-in-script`](examples/malicious/hardcoded-secret-in-script) | A bundled script hardcoding a private-key-shaped credential block | Needs a curated issuer-format pattern list most static scanners don't ship at all, let alone tuned against known placeholders | HIGH (score 7): `hardcoded_secret` |
| [`malicious/typosquatted-dependency`](examples/malicious/typosquatted-dependency) | A setup command installing `reqeusts`, a one-character transposition of the popular PyPI package `requests` | Needs name-similarity matching against a popular-package list, not a pattern most static scanners carry | MEDIUM (score 3): `dependency_typosquat`, calibrated below CRITICAL since a static check can't tell a typosquat from a careless author typo |
| [`malicious/known-malicious-dependency`](examples/malicious/known-malicious-dependency) | A `requirements.txt` pinning `guardrails-ai==0.10.1`, a real, documented supply-chain compromise (GHSA-xmpw-2vmm-p4p6) — not a hypothetical | Needs a curated, GHSA-cited known-malicious-package list, exact-version-aware, not a pattern most static scanners carry | CRITICAL (score 15): `known_malicious_package`, calibrated above `dependency_typosquat` since this is an exact documented match, not a probabilistic name guess |
...
5. **Differential execution** (opt-in via `--differential`). Re-runs each invocation candidate a second time with a different container hostname, a couple of interactive-session-looking env vars (`TERM`, `SSH_CONNECTION`), and desktop-plausible container resource limits (tight `--cpus`/`--memory`/`--pids-limit` by default, loosened on the varied pass), then diffs the two runs' network/file/subprocess signatures. Behavior that shows up in only one of the two runs is exactly the signal a sandbox-aware skill produces. Off by default, since it roughly doubles sandbox runtime; see [Scope and limitations](#scope-and-limitations-v1) for how far this does and doesn't go.
...
- **`--differential` varies hostname, a couple of env vars, and container resource limits — not timing or filesystem artifacts.** That catches a skill branching on the most obvious environment and resource signals, not a deliberate, well-resourced evasion check (real machine ID, uptime, `/.dockerenv`, installed-package fingerprints, timing-based stalling). Deepening this further is the next step under [Roadmap](#roadmap)'s sandbox-evasion-resistance item, not a solved problem.
...
For a periodic re-scan of an already-cleared skill (a `schedule:`-triggered workflow, say) rather than a per-PR gate, save each scan's `--json`/`-o` report and pass the previous one to the next run via `--compare-to prior-report.json` — it diffs findings against that prior scan and highlights specifically what's new, so a skill that goes bad after publication (a self-updating `git pull` skill, for instance) doesn't require diffing two full reports by hand. See [Roadmap](#roadmap)'s re-scan-on-update item for what this does and doesn't cover.
...
| `frontmatter_sensitive_path_scope` | MEDIUM | Deterministic pattern match against a fixed token list, but the scope itself can be entirely legitimate for a real dotfile-hygiene helper | [T1552.001](https://attack.mitre.org/techniques/T1552/001/) Unsecured Credentials: Credentials In Files |
| `hidden_unicode_content` | HIGH | Deterministic character-code check, virtually no legitimate use for these codepoints in hand-written skill prose (ZWJ/ZWNJ deliberately excluded to avoid emoji/script false positives) | [T1027](https://attack.mitre.org/techniques/T1027/) Obfuscated Files or Information |
| `hardcoded_secret` | HIGH | Deterministic, structural prefix match against 8 known issuer formats, narrowed with an AWS-example allowlist plus a placeholder/low-diversity filter | [T1552.001](https://attack.mitre.org/techniques/T1552/001/) Unsecured Credentials: Credentials In Files |
| `dependency_typosquat` | MEDIUM | Probabilistic name-similarity match (`difflib`) against a small, hand-curated popular-package list — not validated against a real corpus of typosquat incidents | [T1195.001](https://attack.mitre.org/techniques/T1195/001/) Compromise Software Dependencies and Development Tools |
| `known_malicious_package` | CRITICAL | Exact match (name, or name + pinned version) against a small, GHSA-cited list of documented real compromises — not a probabilistic guess, but only as complete as the list itself | [T1195.001](https://attack.mitre.org/techniques/T1195/001/) Compromise Software Dependencies and Development Tools |
...
- **Supply-chain / dependency analysis.** Two slices now: `dependency_typosquat` checks install targets in `pip`/`npm`/`yarn` commands and bundled `requirements.txt`/`package.json` manifests for name-similarity to a small, hand-curated list of popular packages; `known_malicious_package` (see [Explainability](#explainability)) checks the same surfaces for an exact match — name, or name plus pinned version — against `sentinel/known_malicious_packages.py`, a small (~20-entry) list where every entry is individually sourced from a real, human-reviewed GitHub Security Advisory and cited by GHSA ID in the finding itself. Deliberately not a bulk import of an external feed: GHSA's own `type:malware` corpus and OpenSSF's `malicious-packages` database are both six-figure-scale firehoses, overwhelmingly automated typosquat/dependency-confusion spam caught within hours — the wrong shape for a hand-curated, individually-explained list, so this stays narrow and honest about it rather than claiming comprehensive coverage. Still open: dependency confusion, and cross-checking what a skill actually pulls in against what it declares in a separate manifest.
- **Sandbox-evasion resistance.** The sandbox has a consistent, in-principle-detectable fingerprint (the mitmproxy CA, the sinkhole behavior). A sufficiently deliberate attacker could check for that and behave clean during scanning, the standard malware-analysis arms race. `--differential` (see [How it works](#how-it-works)) now varies hostname, a couple of env vars, and container resource limits (CPU quota/memory/pids — tight by default, desktop-plausible on the varied pass). Confirmed by hand against a real container rather than assumed: a naive `os.cpu_count()`/`free -m` check sees no difference at all (Docker's `--cpus`/`--memory` aren't reflected in plain `/proc`), only the cgroup limit files directly (`/sys/fs/cgroup/{cpu.max,memory.max,pids.max}`) do — so this specifically catches a skill checking those, a known container-detection technique, not just any resource-aware check. Still open: timing (`/proc/uptime` looking freshly-booted) and filesystem artifacts (`/.dockerenv`, cgroup contents naming `docker`) — both would need a fake `/proc`/filesystem overlay inside the sandbox image, a real architecture change, not a solved problem.
- **Re-scan on update.** A skill can pass review clean and turn malicious later. Several real skills scanned here have self-update mechanisms (`git pull`, checking their own `SKILL.md` on GitHub). Point-in-time scanning doesn't catch a skill going bad after publication — SkillTrace itself stays a stateless CLI (no daemon, no scheduler), so the actual periodic re-scanning is external (cron, a GitHub Actions `schedule:` trigger); `--compare-to <prior-report.json>` is what makes a re-scan worth running: it diffs the new scan's findings against a prior one and highlights specifically what's new, matched by skill name (not path — a git-cloned skill lands in a fresh temp directory every run, so paths never match run-to-run) and by a normalized finding signature (so re-cloning doesn't make every finding look new just because the absolute path changed). Deliberately not wired into `--fail-threshold` yet: gating CI specifically on new-since-last findings, versus the whole report's risk level, is a real design question left open, not assumed.
